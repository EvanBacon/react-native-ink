...
const adjustProps = (domProps: any) => {
  const {
    //   onClick,
    onResponderRelease,
    role,
  ...
  if (isLinkRole && onResponderRelease) {
    domProps.onClick = function(e: any) {
      if (
  ...
  //   if (isButtonLikeRole && !isDisabled) {
  //     domProps.onKeyPress = function(e: any) {
...
const createElement = (component: any, props: any, ...children: any[]): any => {
  // use equivalent platform elements where possible
  let accessibilityComponent: any;
  if (component && component.constructor === String) {
    // accessibilityComponent = AccessibilityUtil.propsToAccessibilityComponent(props);
  }
  const Component = accessibilityComponent || component;
  const domProps = props; //createDOMProps(Component, props);
