...
export default class Element extends Node {
  ...
  set draggable(draggable) {
    draggable ? this.enableDrag(draggable) : this.disableDrag();
  }
  ...
  onceScreenEvent(type: string, handler: Function) {
    const listeners = (this._slisteners = this._slisteners || []);
  ...
  removeScreenEvent(type: string, handler: Function) {
    const listeners = (this._slisteners = this._slisteners || []);
  ...
  setText(content: string = '', noClear?: any) {
  ...
  parseContent(noTags?: any) {
    ...
    if (
      ...
      this._clines.ci = [];
      this._clines.reduce((total, { length }) => {
  ...
  setBaseLine(i: number, line: number) {
    const fake = this._clines.rtof[this.childBase || 0];
